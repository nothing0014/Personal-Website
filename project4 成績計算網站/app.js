...
// 選擇select 內的option之後要改變相對應的顏色
let allselcts = document.querySelectorAll("select");
allselcts.forEach((select) => {
  select.addEventListener("change", (e) => {
    setGPA();
    changeColor(e.target); //e.targe就是<select>
  });
});
...
function setGPA() {
  ...
  let totalScore = 0; //GPA計算用分子
  let totalCredits = 0; //GPA計算用分母

  //計算分母
  for (let i = 0; i < credits.length; i++) {
    if (!isNaN(credits[i].valueAsNumber)) {
      totalCredits += credits[i].valueAsNumber;
    }
  }
  for (let i = 0; i < formLength; i++) {
    if (!isNaN(credits[i].valueAsNumber))
      totalScore += credits[i].valueAsNumber * convertor(selects[i].value);
  }
  let result;
  if (totalCredits == 0) {
    result = (0.0).toFixed(2);
  } else {
    result = (totalScore / totalCredits).toFixed(2);
  }
...
<i class="fas fa-trash"></i>
  ...
  //select顏色轉換
  allselcts = document.querySelectorAll("select");
  allselcts.forEach((select) => {
    changeColor(select);
    select.addEventListener("change", (e) => {
      setGPA();
